describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const innerText = await (await count[0].getProperty('innerText')).jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i=0; i<prodItems.length; i++)
    {
      const shadowRoot = await prodItems[i].getProperty('shadowRoot');
      const button = await shadowRoot.$$('button');
      let innerText = await (await button[0].getProperty('innerText')).jsonValue();
      expect(innerText).toBe('Remove from Cart');
    }
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const localCart = await page.evaluate(() => localStorage.getItem('cart'));
